...
describe('useDebounce tests', () => {
  ...
  it('is a generic type, boolean', () => {
    ...
    expect(typeof booleanResult.current[0]).toStrictEqual('boolean');
  });

  it('is a generic type, string', () => {
    const { result: stringResult } = getHook('true');
    expect(typeof stringResult.current[0]).toStrictEqual('string');
  });

  it('is a generic type, number', () => {
    const { result: numberResult } = getHook(15);
    expect(typeof numberResult.current[0]).toStrictEqual('number');
  });
  ...
  it('it keeps debouncing the value if the value is repeatedly updated', () => {
    ...
    expect(result.current[0]).toStrictEqual('first');
    ...
    expect(result.current[0]).toStrictEqual('third');
  });
});
